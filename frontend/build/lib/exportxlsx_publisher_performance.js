  ...
  if (data.length) {
    $.each(data, function (tblNo, tabledata) {
      if (tabledata.data.length) {
        ...
        $.each(tabledata.selectedFilters, function (hkey, hvalue) {
          R = hkey;
          if (range.s.r > R) range.s.r = R;
          if (range.s.c > C) range.s.c = C;
          if (range.e.r < R) range.e.r = R;
        ...
        $.each(tabledata.data, function(key, value) {
            $.each(header, function(hkey, val) {
              ...
              if(val.format == "number") {
                ...
                cell.v = parse === false ? defaultValue : parseInt(currentValue, 10);
                cell.z = '#,##0';
                cell_ref = XLSX.utils.encode_cell({c: C, r: R});
                if (typeof cell.v === 'number') cell.t = 'n';
                else cell.t = 's';
                ...
                cell.s = val.hasOwnProperty('styleinfo') == true ? styles[val.styleinfo.tdata] : styles.default;
                cell.v = parse === false ? defaultValue : parseFloat(currentValue);
                cell.z = '$#,##0.00';
                cell_ref = XLSX.utils.encode_cell({c: C, r: R});
                if (typeof cell.v === 'number') cell.t = 'n';
                else cell.t = 's';
                ...
                cell.s = val.hasOwnProperty('styleinfo') == true ? styles[val.styleinfo.tdata] : styles.default;
                cell.v = parse === false ? defaultValue : datenum(new Date(currentValue));
                ...
                cell.s = val.hasOwnProperty('styleinfo') == true ? styles[val.styleinfo.tdata] : styles.default;
                cell.v = parse === false ? defaultValue : parseFloat(currentValue)/100;
                cell.z = "0.00%";
                cell_ref = XLSX.utils.encode_cell({c: C, r: R});
                if (typeof cell.v === 'number') cell.t = 'n';
                else cell.t = 's';
